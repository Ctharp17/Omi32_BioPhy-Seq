...
var annotate_variants = {original: 'UCA',
                         variant: 'Omi32'};
...
var main_svg;
var use_data;
var kd_data = {'Expression': {}, 'BA4': {}, 'BA1': {}, 'WUHAN': {}, 'PSR': {}};
var kd_conc_map = {
  'Expression': null,
  'BA4': [0, -12, -11, -10, -9, -8, -7],
  'BA1':[0, -12, -11, -10, -9, -8, -7],
  'WUHAN': [0, -12, -11, -10, -9, -8, -7],
  'PSR': [0, -12, -11, -10, -9, -8, -7, -6, -5]
}
...
var x_by_kd = d3.scaleLinear().domain([7,11]).range([630,790]);
var color_by_kd = d3.scaleSequential(d3.interpolateViridis).domain([7,11]);
...
var scale_y_violin =  {'Expression': 0.1, 'BA4': 0.1, 'BA1': 0.1,
                       'WUHAN': 0.1,
                       'PSR': 0.1};
...
function draw_data() {
  ...
  for (let d of use_data) {
    ...
    // Only draw violin plot point if has_kd is true
    if (d['has_kd']) {
...
function draw_brush_only() {
  for (let d of use_data) {
    ...
    for (let i=0; i<4; i++) {
      for (j=0; j<4; j++) {
        if (!(((i==0) || (i==3)) && ((j==0) || (j==3)))) {
          let index = ((x-1+i) + (y-1+j) * canvasWidth) * 4;
          canvasData.data[index + 3] = a;
        }
      }
    }
...
function filter_data() {
  use_data = main_data.filter(function(d) {
    // First check if Kd value exists for current antigen
    if (!d['has_kd']) return false;
    
    // Then apply allele filters
...
function setup_interaction() {
  ...
  */
  ...
  const UCA_LABEL_DX = -20;
  const UCA_LABEL_DY = -120;
  const OMI_LABEL_DX = 20;
  const OMI_LABEL_DY = 80;

  
  const uca_x = xs(data_by_variant[original]['fdl_x']);
  const uca_y = ys(data_by_variant[original]['fdl_y']);
  
  main_svg.append('text')
  	.attr('class', 'germ_som_text')
  	.attr('x', uca_x + UCA_LABEL_DX)
  	.attr('y', uca_y + UCA_LABEL_DY)
  	.text('UCA');
  ...
  main_svg.append('line')
  	...
  	.attr('stroke', '#555')
  	.attr('stroke-width', 1)
  	.attr('x1', uca_x + UCA_LABEL_DX + 10) // small nudge toward text center
  	.attr('y1', uca_y + UCA_LABEL_DY + 10)
  	.attr('x2', uca_x)
  	.attr('y2', uca_y);

  const omi_x = xs(data_by_variant[variant]['fdl_x']);
  const omi_y = ys(data_by_variant[variant]['fdl_y']);
  
  main_svg.append('text')
  	.attr('class', 'germ_som_text')
  	.attr('x', omi_x + OMI_LABEL_DX)
  	.attr('y', omi_y + OMI_LABEL_DY)
  	.text('Omi32');
  ...
  main_svg.append('line')
  	...
  	.attr('stroke', '#555')
  	.attr('stroke-width', 1)
  	.attr('x1', omi_x + OMI_LABEL_DX - 5)
  	.attr('y1', omi_y + OMI_LABEL_DY - 5)
  	.attr('x2', omi_x)
  	.attr('y2', omi_y);
...
function get_violin_y(xpos, d) {
  // Don't calculate violin position for missing values
  if (!d['has_kd']) {
    return -100; // Return off-canvas position
  }
...
function kd_for(kd_var_tmp) {
  ...
  if (kd_var == 'BA4') {
    ...
    color_by_kd = d3.scaleSequential(d3.interpolateViridis).domain([7,11]);
    d3.select("#x_axis_label_violin").text("-log10(Kd)"); // Fixed typo
  }
  else if (kd_var == 'BA1') {
    x_by_kd = d3.scaleLinear().domain([7,11]).range([630,790]);
    color_by_kd = d3.scaleSequential(d3.interpolateViridis).domain([7,11]);
  ...
  else if (kd_var == 'WUHAN') {
    x_by_kd = d3.scaleLinear().domain([7,11]).range([630,790]);
    color_by_kd = d3.scaleSequential(d3.interpolateViridis).domain([7,11]);
    d3.select("#x_axis_label_violin").text("-log10(Kd)");
  }
  else if (kd_var == 'PSR') {
    x_by_kd = d3.scaleLinear().domain([0,3]).range([630,790]);
    color_by_kd = d3.scaleSequential(d3.interpolateViridis).domain([0,3]);
    d3.select("#x_axis_label_violin").text("-log10(Kd)");
  }
  ...
  for (let d of main_data) {
    // Check if value is missing (empty string or "null")
    let kdValue = d[kd_var+'_log10Kd'];
    let hasMissingKd = (kdValue === '' || kdValue === 'null' || kdValue === null || kdValue === undefined);
    
    if (hasMissingKd) {
      // Mark as missing - position off-canvas and color gray
      d['kdx'] = -100; // Position outside visible area
      d['kd_color'] = d3.color("rgba(150, 150, 150, 0.3)"); // Gray color
      d['has_kd'] = false;
    } else {
      d['kdx'] = Math.floor(x_by_kd(Number(kdValue) * (kd_var != 'Expression' ? -1 : 1)));
      d['kd_color'] = d3.color(color_by_kd(Number(kdValue) * (kd_var != 'Expression' ? -1 : 1)));
      d['has_kd'] = true;
    }
    
    // Handle delta Kd values
    for (let i=0; i<alleles.length; i++) {
      let deltaValue = d[kd_var + '_' + i + '_deltaKd'];
      if(deltaValue == "null" || deltaValue === '' || deltaValue === null || deltaValue === undefined){
        d['delta_kd_' + i] = d3.color("rgba(200, 200, 200, 0.1)");
      }
      else{
        d['delta_kd_' + i] = d3.color(color_by_delta(Number(deltaValue)));
      }
    }
  }
...
function setup_viz() {
  ...
  if (x_d_dif > y_d_dif) {
    y_domain[0] = y_domain[0] - (x_d_dif-y_d_dif)/2 - x_d_dif/20;
  ...
  for (let d of main_data) {
    ...
    d['x'] = Math.floor(d['x_exact']);
    d['y'] = Math.floor(d['y_exact']);
    
    // Check if initial Kd value is missing
    let kdValue = d[kd_var+'_log10Kd'];
    let hasMissingKd = (kdValue === '' || kdValue === 'null' || kdValue === null || kdValue === undefined);
    
    if (hasMissingKd) {
      d['kdx'] = -100;
      d['kd_color'] = d3.color("rgba(150, 150, 150, 0.3)");
      d['has_kd'] = false;
    } else {
      d['kdx'] = Math.floor(x_by_kd(Number(kdValue)*-1));
      d['kd_color'] = d3.color(color_by_kd(Number(kdValue)*-1));
      d['has_kd'] = true;
    }
...
function read_files(fpath) {
  ...
  d3.csv(fpath).then(function(data) {
    ...
    aq.loadArrow('data/BA4.arrow').then((td) => kd_data['BA4']=td);
    aq.loadArrow('data/BA1.arrow').then((td) => kd_data['BA1']=td);
    aq.loadArrow('data/WUHAN.arrow').then((td) => kd_data['WUHAN']=td);
    aq.loadArrow('data/PSR.arrow').then((td) => kd_data['PSR']=td)
