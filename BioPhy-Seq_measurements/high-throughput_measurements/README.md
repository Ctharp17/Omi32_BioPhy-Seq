...
build the environment with 'conda env create -f environment.yaml -n omicron' , please note that this environment configuration file is saved using the --from-history flag so it should enable automatic package handling between linux and MacOS.


```

Then activate conda with 'conda activate omicron'
