### use regexes generated earlier to align each paired Illumina read, throwing away both reads if either fail to align to regexes
    ...
    with gzip.open(snakemake.input.read_1_fastq, 'rt') as read_1_handle:
        with gzip.open(snakemake.input.read_2_fastq, 'rt') as read_2_handle:
            for (read_1, read_2) in zip(
                ...
                # write to file
                fw.write("\t".join(
                    [inline_idx_1, inline_idx_2, UMI, barcode]
                ) + "\n")
